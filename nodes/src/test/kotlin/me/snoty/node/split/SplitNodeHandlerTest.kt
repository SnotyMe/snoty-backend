...
import me.snoty.node.split.SplitBehavior
import me.snoty.node.split.SplitNodeHandler
import me.snoty.node.split.SplitSettings
...
class SplitNodeHandlerTest {
	...
	private suspend fun `replace root impl`() {
		val node = node("split", settings = SplitSettings(key = "key", behavior = SplitBehavior.REPLACE_ROOT))
		val input = listOf(
	...
	private suspend fun `replace key impl`() {
		val node = node("split", settings = SplitSettings(key = "key", behavior = SplitBehavior.REPLACE_KEY))
		val input = listOf(
			mapOf("key" to listOf(Document("value1", true), "value2"), "other" to "sharedValue")
		).map { context.intermediateDataMapperRegistry[BsonIntermediateData::class].serialize(it) }
