...
fun Resolver.getDetails(
	...
	return when {
		Collection::class.isAssignableFrom(type, this) -> {
			...
			SchemaFieldDetails.CollectionDetails(fieldDetails)
		}
		...
		String::class.isAssignableFrom(type, this) ->
			SchemaFieldDetails.PlaintextDetails(
		...
		Map::class.isAssignableFrom(type, this) -> {
			...
			SchemaFieldDetails.MapDetails(keyDetails, valueDetails)
		}
		...
		Any::class.isAssignableFrom(type, this) && !className.packageName.startsWith("java") && !className.packageName.startsWith("kotlin") -> {
			...
			SchemaFieldDetails.ObjectDetails(
...
@OptIn(KspExperimental::class)
private fun Resolver.getEnumDetails(prop: KSPropertyDeclaration): SchemaFieldDetails.EnumDetails {
	...
	val elements = test.declarations
		...
		.map {
			...
			SchemaFieldDetails.EnumDetails.EnumConstant(value, displayName ?: value.toTitleCase())
		}
		.toList()
	return SchemaFieldDetails.EnumDetails(elements)
}
