...
fun setupFeatureFlags(featureClient: Client, featureFlags: Collection<LogFeatureFlagsContainer>) {
...
private fun loggerFeatureFlagListener(client: Client, flag: LogLevelFeatureFlag) =
	FeatureFlagChangeListener(client, flag, true) {
		flag.loggers.forEach { loggerName ->
			...
			if (target == null) {
				logger.warn { "Logger for flag ${flag.name}, package $loggerName not found" }
				return@FeatureFlagChangeListener
			}

			target.level = Level.convertAnSLF4JLevel(it)
			logger.info { "Set log level for flag ${flag.name}, package $loggerName to $it" }
		}
	}
